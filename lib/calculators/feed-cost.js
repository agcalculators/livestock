...
export default function({startWeight, endWeight, daysOnFeed, totalFeedCost }) {
    let errors = {
        daysOnFeed: validate(daysOnFeed, ['required', 'number', 'min'], { min: 1}),
        totalFeedCost: validate(totalFeedCost, ['required', 'number', 'min'], { min: 0 }),
        startWeight: validate(startWeight, ['required', 'number', 'min'], { min: 1}),
        endWeight: validate(endWeight, ['required', 'number', 'gt'], { gt: 'startWeight' }, { startWeight }),
    };
