...
export default function({gender, birthWeight, currentWeight, currentAge, ageOfDam}) {
    let errors = {
        gender: validate(gender, ['required', 'oneOf'], { oneOf: ['male', 'female'] }),
        birthWeight: validate(birthWeight, ['required', 'number', 'min'], { min: 1 }),
        currentWeight: validate(currentWeight, ['required', 'number', 'min', 'gt'], { min: 1, gt: 'birthWeight' }, { birthWeight }),
        currentAge: validate(currentAge, ['required', 'number', 'min', 'max'], { min: 150, max: 300}),
