...
export default {
    ...
    calendar: {
        adjustedYearlingWeight: ({calculated, adjustedYearlingWeight, units = 'lbs'}) => ({ subject: 'Adjusted 365 Day Yearling Weight', details: `${adjustedYearlingWeight} ${units} ${adjustment > 0 ? `(+${adjustment} ${units})` : ''}`, from: calculated })
    },
    dashboard: {
        adjustedYearlingWeight: { 
            ...
            params: { 
                ...
                measure:  ({adjustedYearlingWeight, units = 'lbs'}) => `${adjustedYearlingWeight} ${units}`,
                units: 'lbs',
                source: 'calculator',
                details: ({calculated, formatters}) => 
